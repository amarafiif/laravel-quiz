 ...
 <div x-data="{ open: false }" @keydown.escape.window="open = false" class="sticky top-0 z-50 bg-white shadow-sm">
     <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
         class="fixed inset-0 z-40 bg-black/20 backdrop-blur-sm" aria-hidden="true" x-cloak>
     </div>

     <!-- Navbar utama -->
     <nav class="relative z-50 w-full border-b border-gray-200/20 bg-white">
         <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
             <div class="flex h-20 justify-between">
                 ...
                 <div class="hidden items-center space-x-8 md:flex">
                     <a href="{{ route('home') }}" class="{{ request()->routeIs('home') ? 'text-sky-500' : '' }} px-3 py-2 font-medium text-gray-600 transition-colors hover:text-sky-500">Beranda</a>
                     <a href="{{ route('list.quizzes') }}" class="{{ request()->routeIs('list.quizzes') ? 'text-sky-500' : '' }} px-3 py-2 font-medium text-gray-600 transition-colors hover:text-sky-500">Kuis</a>
                     <a href="/#about" class="px-3 py-2 font-medium text-gray-600 transition-colors hover:text-sky-500">Tentang</a>
                     <a href="#" class="rounded-lg bg-sky-500 px-8 py-2 font-medium text-white transition-colors hover:bg-sky-600">Daftar</a>
                 </div>
         ...
         <!-- Mobile Navigation -->
         <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform -translate-y-4" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-200"
             x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-4" class="absolute w-full md:hidden" @click.away="open = false" x-cloak>
             <div class="space-y-1 border-t border-gray-200 bg-white px-2 pb-3 pt-2 shadow-lg">
                 <a href="{{ route('home') }}" class="{{ request()->routeIs('home') ? 'text-sky-500' : '' }} block rounded-md px-3 py-2 font-medium text-gray-600 hover:bg-sky-100 hover:text-sky-500">Beranda</a>
                 <a href="{{ route('list.quizzes') }}" class="{{ request()->routeIs('list.quizzes') ? 'text-sky-500' : '' }} block rounded-md px-3 py-2 font-medium text-gray-600 hover:bg-sky-100 hover:text-sky-500">Kuis</a>
                 <a href="#about" class="block rounded-md px-3 py-2 font-medium text-gray-600 hover:bg-gray-100 hover:text-sky-500">Tentang</a>
                 <div class="mt-3 border-t border-gray-200/50 pt-3">
                     <a href="#" class="block w-full rounded-lg bg-sky-500 px-3 py-2 text-center font-medium text-white hover:bg-sky-600">Daftar</a>
                 </div>
             </div>
         </div>
     </nav>
