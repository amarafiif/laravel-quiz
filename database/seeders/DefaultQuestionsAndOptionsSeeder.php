...
class DefaultQuestionsAndOptionsSeeder extends Seeder
{
    ...
    {
        // Cari quiz berdasarkan nama
        $quizPemrogramanDasar = Quiz::where('name', 'Kuis Pemrograman Dasar')->first();

        if (! $quizPemrogramanDasar) {
            $this->command->warn('Quiz "Kuis Pemrograman Dasar" not found. Please run DefaultQuizSeeder first.');
        ...
        $questions = [
            [
                ...
                'quiz_id' => $quizPemrogramanDasar->id,
                'options' => [
        ...
        foreach ($questions as $questionData) {
            ...
            $questionStatus = $question->wasRecentlyCreated ? 'CREATED' : 'UPDATED';
            $this->command->info("Question {$questionStatus}: {$question->question_text} (Quiz: {$quizPemrogramanDasar->name})");

            $existingOptionsCount = $question->options()->count();
            if ($existingOptionsCount > 0) {
                $question->options()->delete();
                $this->command->info("Deleted {$existingOptionsCount} existing options for question: {$question->question_text}");
            }
            ...
            $this->command->info("Created {$createdOptionsCount} options for question: {$question->question_text}");
        }
    }
}
